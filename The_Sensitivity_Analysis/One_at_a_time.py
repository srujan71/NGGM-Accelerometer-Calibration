...
def run_sim(combination, path, inertia_tensor, thruster_arm, a_ng_requirement_power, power_NFFT, power_f_lb, power_f_ub):
    try:
        logging.info(f"Starting simulation: {combination['Folder']} Run {combination['Index']}")
        ...
        output_path = path + f"{folder_name}/Run_{index}/"
        ...
        acc_lst_science_gg = acceleration_setup(shaking_dictionary=shaking_dictionary,
                                             position_dictionary=position_dictionary[axis][layout],
                                             layout=layout, duration=172800, noise_switch=noise_switch,
                                             shake_bool=False, goce_parameters=False,
                                             load_true_data=True, x_true=x_true, acc_lst_dummy=acc_lst, gg=True)

        acc_lst_science_gg_w_rate = acceleration_setup(shaking_dictionary=shaking_dictionary,
                                             position_dictionary=position_dictionary[axis][layout],
                                             layout=layout, duration=172800, noise_switch=noise_switch,
                                             shake_bool=False, goce_parameters=False,
                                             load_true_data=True, x_true=x_true, acc_lst_dummy=acc_lst, gg_w_rate=True)

        # Calculate the estimate of a_ng
        a_ng_rcst = reconstruct_a_ng(x0, acc_lst_science, layout)
        a_ng_rcst_gg = reconstruct_a_ng(x0, acc_lst_science_gg, layout, gg=True)
        a_ng_rcst_gg_w_rate = reconstruct_a_ng(x0, acc_lst_science_gg_w_rate, layout, gg_w_rate=True)


        a_ng_true = acc_lst_science[0].a_ng
        a_ng_true_gg = acc_lst_science_gg[0].a_ng
        a_ng_true_gg_w_rate = acc_lst_science_gg_w_rate[0].a_ng

        a_ng_residual = a_ng_rcst - a_ng_true
        a_ng_residual_gg = a_ng_rcst_gg - a_ng_true_gg
        a_ng_residual_gg_w_rate = a_ng_rcst_gg_w_rate - a_ng_true_gg_w_rate


        a_ng_residual_relative = np.sqrt(2) * a_ng_residual
        a_ng_residual_relative_gg = np.sqrt(2) * a_ng_residual_gg
        a_ng_residual_relative_gg_w_rate = np.sqrt(2) * a_ng_residual_gg_w_rate
        # LoS vector
        LoS = np.array([1, 1e-5, 1e-5])
        LoS_dir = LoS / np.linalg.norm(LoS)
        a_ng_residual_relative_los = np.dot(a_ng_residual_relative, LoS_dir)
        a_ng_residual_relative_los_gg = np.dot(a_ng_residual_relative_gg, LoS_dir)
        a_ng_residual_relative_los_gg_w_rate = np.dot(a_ng_residual_relative_gg_w_rate, LoS_dir)

        a_ng_error_power = calculate_power_band(a_ng_residual_relative_los, power_NFFT, power_f_lb, power_f_ub)
        a_ng_error_power_gg = calculate_power_band(a_ng_residual_relative_los_gg, power_NFFT, power_f_lb, power_f_ub)
        a_ng_error_power_gg_w_rate = calculate_power_band(a_ng_residual_relative_los_gg_w_rate, power_NFFT, power_f_lb, power_f_ub)

        a_ng_error_power_ratio = a_ng_error_power / a_ng_requirement_power
        a_ng_error_power_ratio_gg = a_ng_error_power_gg / a_ng_requirement_power
        a_ng_error_power_ratio_gg_w_rate = a_ng_error_power_gg_w_rate / a_ng_requirement_power

        np.savetxt(output_path + "PSD_ratio.txt", np.array([a_ng_error_power_ratio]))
        np.savetxt(output_path + "PSD_ratio_gg.txt", np.array([a_ng_error_power_ratio_gg]))
        np.savetxt(output_path + "PSD_ratio_gg_w_rate.txt", np.array([a_ng_error_power_ratio_gg_w_rate]))
...
if __name__ == "__main__":
    ...
    for th_idx, th in enumerate(nominal_thrust):
        nominal_settings = {'Thrust': th, 'Duration': 86400, 'Frequency': 1e-1, 'Length': 0.6, 'Bandwidth': 4, 'Layout': 2, 'Axis': 'x'}
        ...
        settings = {'Thrust': [1e-8, 1e-7, settings_th, 1e-4],
                     'Duration': [7200, 21600, 43200, 64800, 108000, 129600, 151200, 172800],
                     'Frequency': [1e-3, 1e-2],
        ...
        # 1) Find the frequency setting
        for freq_level in settings["Frequency"]:
            freq_arr = find_setting("Frequency", freq_level, combinations_lst)
            calculate_factor_frequency(nominal_settings, freq_arr[0])
        # 2) Find the bandwidth setting
        for bandwidth_level in settings["Bandwidth"]:
            bandwidth_arr = find_setting("Bandwidth", bandwidth_level, combinations_lst)
            calculate_factor_frequency(nominal_settings, bandwidth_arr[0])
        ...
        for seed in seeds:
            ...
            np.random.seed(seed)
            logging.info("----------------------------------------")
            logging.info(f"Starting simulations for seed: {seed}")
            logging.info("----------------------------------------")

            start = time.time()

            folder_path = current_dir + f"/../SimulationOutput/Output/Sensitivity_analysis/One_at_a_time/x_axis/Thrust_{th}/Seed_{seed}/"

            with Pool(num_processes) as pool:
                args = [(combination, folder_path, J, M_arm, a_ng_req_power, NFFT, f_lb, f_ub) for combination in combinations_lst]
            ...
            time_dictionary[seed] = total_time

            save2txt(time_dictionary, "Time.txt", folder_path)
